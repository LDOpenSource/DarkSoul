...
namespace DarkSoul.Network
{
    public static class StreamExtensions
    {
        ...
        public static IObserver<ArraySegment<byte>> ToStreamObserver<T>(this T stream, CancellationToken token) where T : BaseClient
        {
            return Observer.Create<ArraySegment<byte>>(async buffer =>
            {
                ...
                {
                    stream.Disconnect();
                    Console.WriteLine($"[Client Error] Message: {error.Message} - Trace: {error.StackTrace}");
                }                
            },
            error =>
            {
